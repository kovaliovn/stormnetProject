 ...
 * Требования:
 * •	Метод runWorkingProcess() должен вызывать pilot(Human human) 2 раза. В качестве аргументов передать все объекты
 * типа Human из списка astronauts.
 * •	Метод runWorkingProcess() должен вызывать createDirection(Dog dog) 1 раз. В качестве аргумента передать объект
 * типа
 * Dog из списка astronauts.
 * •	Метод runWorkingProcess() должен вызывать research(Cat cat) 1 раз. В качестве аргумента передать объект типа
 * Cat из
 * списка astronauts.
 */
